...
class NeoItemsSalesCore extends ObjectModel
{
    // @var integer
    public $id_neo_item_sales;
    ...
    public function delete()
    {
        ...
        Db::getInstance()->delete('neo_items_buys', 'id_neo_item_buy='.(int)$this->id);
    ...
    public static function getDownloadFromHash($hash)
    {
		...
		LEFT JOIN `'._DB_PREFIX_.'product_download` pd ON (od.`id_product`=pd.`id_product`)
    ...
    {
        /*larismendi $sql = 'UPDATE `'._DB_PREFIX_.'order_detail`
    ...
    {
        return NeoItemsBuy::getTaxCalculatorStatic($this->id);
    }
    ...
    {
        /*larismendi $sql = 'SELECT t.*, d.`tax_computation_method`
    ...
    {
        // Nothing to save
        /*larismendi if ($this->tax_calculator == null)
            return true;
     ...
     * @param array $product
     */
    protected function setDetailProductPrice(NeoExchanges $order, Cart $cart, $product)
    {
        $this->setContext((int)$product['id_shop']);
     ...
     */
    protected function create(NeoExchanges $order, Cart $cart, $product, $id_order_state, $id_order_invoice, $use_taxes = true, $id_warehouse = 0)
    {
        if ($use_taxes)
     ...
     * @param bool $use_taxes set to false if you don't want to use taxes
     */
    public function createList(NeoExchanges $order, Cart $cart, $id_order_state, $product_list, $id_order_invoice = 0, $use_taxes = true, $id_warehouse = 0)
    {
     ...
     * @param $product
     */
    public function setShippingCost(NeoExchanges $order, $product)
    {
        $tax_rate = 0;
    ...
    public static function getCrossSells($id_product, $id_lang, $limit = 12)
    {
		...
		SELECT o.id_neo_exchange
		FROM '._DB_PREFIX_.'neo_exchanges o
		LEFT JOIN '._DB_PREFIX_.'neo_items_buys od ON (od.id_neo_exchange = o.id_neo_exchange)
		WHERE/* o.valid = 1 AND*/ od.id_product = '.(int)$id_product);

        if (sizeof($orders))
        {
            $list = '';
            foreach ($orders AS $order)
                $list .= (int)$order['id_neo_exchange'].',';
            $list = rtrim($list, ',');

            $orderProducts = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
				SELECT DISTINCT od.id_product product_id, p.id_product, pl.name, pl.link_rewrite, p.reference, i.id_image, product_shop.show_price, cl.link_rewrite category, p.ean13, p.out_of_stock, p.id_category_default
				FROM '._DB_PREFIX_.'neo_items_buys od
				LEFT JOIN '._DB_PREFIX_.'product p ON (p.id_product = od.id_product)
				'.Shop::addSqlAssociation('product', 'p').'
				LEFT JOIN '._DB_PREFIX_.'product_lang pl ON (pl.id_product = od.id_product'.Shop::addSqlRestrictionOnLang('pl').')
				LEFT JOIN '._DB_PREFIX_.'category_lang cl ON (cl.id_category = product_shop.id_category_default'.Shop::addSqlRestrictionOnLang('cl').')
				LEFT JOIN '._DB_PREFIX_.'image i ON (i.id_product = od.id_product)
				WHERE od.id_neo_exchange IN ('.$list.')
					AND pl.id_lang = '.(int)$id_lang.'
					AND cl.id_lang = '.(int)$id_lang.'
					AND od.id_product != '.(int)$id_product.'
					AND i.cover = 1
